...
<?php if ( ! have_posts() ) : ?>
	<article id="post-0" class="post no-results not-found">
		<header class="entry-header">
			<h1 class="entry-title"><?php _e( 'Nothing Found', 'tcp-bootstrap-ecommerce' ); ?></h1>
		</header><!-- .entry-header -->

		<div class="entry-content">
			<p><?php _e( 'Apologies, but no results were found for the requested archive. Perhaps searching will help find a related post.', 'tcp-bootstrap-ecommerce' ); ?></p>
			<?php get_search_form(); ?>
		</div><!-- .entry-content -->
	</article><!-- #post-0 -->
<?php endif; ?>
		...
		<?php endif; ?>
				<article <?php post_class(); ?>>
					...
					<?php if ( $see_meta_utilities ) : ?>
						...
						<span class="comments-link btn btn-small"><?php comments_popup_link( '<span class="leave-reply">' . __( 'Leave a review', 'tcp-bootstrap-ecommerce' ) . '</span>', __( '<b>1</b> Review', 'tcp-bootstrap-ecommerce' ), __( '<b>%</b> Reviews', 'tcp-bootstrap-ecommerce' ) ); ?></span>
						<?php endif; // End if comments_open() ?>

						<?php edit_post_link( __( 'Edit', 'tcp-bootstrap-ecommerce' ), '<span class="edit-link">', '</span>' ); ?>
